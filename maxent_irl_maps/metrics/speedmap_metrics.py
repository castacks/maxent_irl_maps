...
from maxent_irl_maps.utils import get_state_visitations, quat_to_yaw
    ...
    with torch.no_grad():
        for i in range(0, len(experiment.dataset), frame_skip):
            ...
            if experiment.categorical_speedmaps:
                ...
                _speeds = (
                    experiment.network.speed_bins[1:]
                ...
                speedmap_prob = sdist_flat[torch.arange(len(eidxs_flat)), eidxs_flat][
                    mask_flat
                ]
                ...
                res = experiment.network.ensemble_forward(map_features)
                speedmap_val = res["speedmap"][0, 0]
                speedmap_err = (speedmap_val - esm).abs()[mask]
                metrics_res["speedmap_l1_err"].append(speedmap_err.mean())
