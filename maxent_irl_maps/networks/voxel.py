...
class VoxelResnetCategorical(torch.nn.Module):
    ...
    """
    def __init__(self, in_channels, bev_in_channels, voxel_params, resnet_params, no_voxel_branch, device):
        super(VoxelResnetCategorical, self).__init__()
        self.in_channels = in_channels
        self.bev_in_channels = bev_in_channels
        self.no_voxel_branch = no_voxel_branch
        self.device = device
    ...
    def setup_bev_net(self, resnet_params):
        n_channels = self.bev_in_channels if self.no_voxel_branch else self.voxel_net[-1].out_channels + self.bev_in_channels
        return ResnetCategorical(**resnet_params, in_channels=n_channels).to(self.device)
