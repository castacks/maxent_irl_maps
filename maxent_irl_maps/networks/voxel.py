...
class VoxelResnetCategorical(torch.nn.Module):
    ...
    """
    def __init__(self, in_channels, voxel_params, resnet_params, device):
    ...
    def predict(self, voxel_grid, return_mean_entropy=False):
        """
        Does all the ros_torch_converter wrapper stuff
        """
        voxel_spconv = voxel_grid_to_spconv(voxel_grid)

        res = self.forward(voxel_spconv, return_mean_entropy)

        return res

    def forward(self, voxel_spconv, return_mean_entropy):
    ...
    def setup_bev_net(self, resnet_params):
        return ResnetCategorical(**resnet_params, in_channels=self.voxel_net[-1].out_channels).to(self.device)
