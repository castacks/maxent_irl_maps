...
class PlannerIRLSpeedmaps:
    ...
    def gradient_step(self, batch):
        ...
        """
        assert (
            batch["metadata"]["resolution"].std() < 1e-4
        ...
        angles = torch.linspace(
            0.0,
            2 * np.pi,
            self.planner.primitives["angnum"] + 1,
            device=self.planner.device,
        ).view(1, -1)

        sgx = (
            (
                (initial_pos[:, 0] - map_params["origin"][:, 0])
                / self.planner.primitives["lindisc"]
            )
            .round()
            .clip(0, self.planner.states.shape[0] - 1)
        )
        sgy = (
            (
                (initial_pos[:, 1] - map_params["origin"][:, 1])
                / self.planner.primitives["lindisc"]
            )
            .round()
            .clip(0, self.planner.states.shape[1] - 1)
        )
        sga = (initial_pos[:, [2]] - angles).abs().argmin(
            dim=-1
        ) % self.planner.primitives["angnum"]
        start_state = torch.stack([sgx, sgy, sga], dim=-1).long()

        # setup goal state
        ggx = (
            (
                (goal_pos[:, 0] - map_params["origin"][:, 0])
                / self.planner.primitives["lindisc"]
            )
            .round()
            .clip(0, self.planner.states.shape[0] - 1)
        ...
        ggy = (
            (
                (goal_pos[:, 1] - map_params["origin"][:, 1])
                / self.planner.primitives["lindisc"]
            )
            .round()
            .clip(0, self.planner.states.shape[1] - 1)
        ...
        for bi in range(start_state.shape[0]):
            ...
            self.planner.load_costmap(
                costmaps[bi].T.detach(), length_weight=self.length_weight
            )
            solution = self.planner.solve(
                goal_states=goal_state[bi].unsqueeze(0), max_itrs=1000
            )
            traj = self.planner.extract_solution_parallel(
                solution, start_state[bi].unsqueeze(0)
            )
            ...
            footprint_learner_traj = apply_footprint(
                traj.unsqueeze(0), self.footprint
            ).view(1, -1, 2)
    ...
    def visualize(self, idx=-1):
        ...
        with torch.no_grad():
            ...
            sga = (initial_pos[2] - angles).abs().argmin() % self.planner.primitives[
                "angnum"
            ]
            start_state = torch.stack([sgx, sgy, sga], dim=-1).long()
            ...
            gga = (goal_pos[2] - angles).abs().argmin() % self.planner.primitives[
                "angnum"
            ]
    ...
    def project_traj_into_fpv(self, traj, init_pose, image):
        ...
        I = get_intrinsics(K)
        E = np.eye(4)
        E[:3, :3] = Rotation.from_quat(
            np.array(self.expert_dataset.config["extrinsics"]["q"])
        ).as_matrix()
        ...
        traj_pts = np.concatenate([traj[:, :3], np.ones([traj.shape[0], 1])], axis=-1)
        local_traj_pts = np.linalg.inv(init_pose_htm).reshape(
            1, 4, 4
        ) @ traj_pts.reshape(-1, 4, 1)
        ...
        (
            ...
            local_traj_pts, pixel_coords, image.shape[0], image.shape[1]
        )
