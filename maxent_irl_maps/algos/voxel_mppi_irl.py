...
class VoxelMPPIIRL:
    ...
    def update(self, n=-1):
        ...
        idxs = np.random.permutation(np.arange(len(self.expert_dataset)))
        max_b = int(idxs.shape[0] / self.batch_size)
        if n < 0:
            n = max_b

        for i in range(min(max_b, n)):
            ...
            batch = [self.expert_dataset[idx] for idx in batch_idxs]
            batch = {k:[x[k] for x in batch] for k in batch[0].keys()}
            for k in ['image', 'traj', 'steer']:
    ...
    def gradient_step(self, batch):
        ...
        map_params = {
            "origin":torch.stack([vg.metadata.origin[:2] for vg in batch['voxel_grid']], dim=0),
            "length":torch.stack([vg.metadata.length[:2] for vg in batch['voxel_grid']], dim=0),
            "resolution":torch.stack([vg.metadata.resolution[:2] for vg in batch['voxel_grid']], dim=0),
        }
        ...
        for bi in range(expert_traj.shape[0]):
            map_params_b = {
                "resolution": map_params["resolution"][bi],
                "length": map_params["length"][bi],
                "origin": map_params["origin"][bi],
            }
        ...
        # bin expert speeds
        _sbins = self.network.bev_net.speed_bins[:-1].to(self.device).view(1, -1, 1, 1)
        sdiffs = expert_speedmaps - _sbins
        sdiffs[sdiffs < 0] = 1e10
        expert_speed_idxs = sdiffs.argmin(dim=1) + 1
        expert_speed_idxs = expert_speed_idxs.clip(0, self.network.bev_net.speed_nbins-1).long()
    ...
    def visualize(self, idx=-1):
        ...
        with torch.no_grad():
            ...
            metadata = {
                "origin": data["voxel_grid"].metadata.origin[:2],
                "length": data["voxel_grid"].metadata.length[:2],
                "resolution": data["voxel_grid"].metadata.resolution[:2],
            }
            ...
            costmap_quantile = compute_speedmap_quantile(costmap_cdf[0], self.network.bev_net.cost_bins, q=0.8).exp()
