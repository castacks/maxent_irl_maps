...
class GlobalStateVisitationBuffer:
    ...
    def get_state_visitations(self, poses, crop_params, local=True):
        ...
        # [B x C x W x H]
        values = self.data[
            pxlist_flipped[:, 0], pxlist_flipped[:, 1]
        ]  # Notice axes are flipped to account for terrain body centric coordinates.
        values = values.view(poses.shape[0], crop_nx, crop_ny)

        k1 = invalid_mask.float()
        values = ((1.0 - k1) * values + k1 * fill_value).swapaxes(-1, -2)
    ...
    def create_anim(self, save_to, local=False):
        ...
        def get_frame(t, fig, axs):
            ...
            axs[1].arrow(0.0, 0.0, 5.0, 0.0, color="r", head_width=1.0)
