...
class PreprocessPointpillarsDataset(Dataset):
    ...
    def __init__(
        self, preprocess_fp, gridmap_type, feature_mean=None, feature_std=None
    ):
        """
        Args:
        """
    ...
    def initialize_dataset(self):
        ...
        if preprocess:
            nfeats = (
                len(
                    torch.load(self.fps[0])["{}_feature_keys".format(self.gridmap_type)]
                )
                if self.gridmap_type == "learned_gridmap"
                else len(
                    torch.load(self.fps[0])["{}_feature_keys".format(self.gridmap_type)]
                )
                - 4
        ...
        if preprocess:
            ...
            for i, fp in enumerate(self.fps):
                ...
                mapfeats = (
                    traj["{}_data".format(self.gridmap_type)]
                    if self.gridmap_type == "learned_gridmap"
                    else traj["{}_data".format(self.gridmap_type)][4:]
                )
                ...
                mean_new = (self.feature_mean + (mapfeats.sum(dim=-1) / K)) * (
                    K / (K + k)
                )
    ...
    def __getitem__(self, idx):
        ...
        data = {
            ...
            if self.gridmap_type == "learned_gridmap"
            else dpt["{}_data".format(self.gridmap_type)][4:],
            "metadata": {
                k: v
                for k, v in dpt["{}_metadata".format(self.gridmap_type)].items()
                if k != "feature_keys"
            },
            "steer": dpt["steer"],
            "traj": dpt["traj"],
            ...
            if self.gridmap_type == "learned_gridmap"
            else dpt["{}_feature_keys".format(self.gridmap_type)][4:],
        }

        data["map_features"] = (
            (data["map_features"] - self.feature_mean.view(-1, 1, 1))
