...
class TrajectoryInterpolator:
    ...
    def __init__(self, times, traj, tol=1e-1, interp_kwargs={}):
        ...
        """
        assert len(traj.shape) == 2, "Expected traj of shape [T x 7/13], got {}".format(
            traj.shape
        )
        assert traj.shape[-1] in [
            7,
            13,
        ], "Expected traj of shape [T x 7/13], got {}".format(traj.shape)
        assert (
            times.shape[0] == traj.shape[0]
        ), "Got {} times, but {} steps in traj".format(times.shape[0], traj.shape[0])
        ...
        times = np.concatenate(
            [np.array([times[0] - self.tol]), times, np.array([times[-1] + self.tol])]
        )
        ...
        self.interp_x = scipy.interpolate.interp1d(
            times[idxs], traj[idxs, 0], **interp_kwargs
        )
        self.interp_y = scipy.interpolate.interp1d(
            times[idxs], traj[idxs, 1], **interp_kwargs
        )
        self.interp_z = scipy.interpolate.interp1d(
            times[idxs], traj[idxs, 2], **interp_kwargs
        )

        rots = scipy.spatial.transform.Rotation.from_quat(traj[:, 3:7])
        self.interp_q = scipy.spatial.transform.Slerp(
            times[idxs], rots[idxs], **interp_kwargs
        )

        if self.is_velocity:
            self.interp_vx = scipy.interpolate.interp1d(
                times[idxs], traj[idxs, 7], **interp_kwargs
            )
            self.interp_vy = scipy.interpolate.interp1d(
                times[idxs], traj[idxs, 8], **interp_kwargs
            )
            self.interp_vz = scipy.interpolate.interp1d(
                times[idxs], traj[idxs, 9], **interp_kwargs
            )

            self.interp_wx = scipy.interpolate.interp1d(
                times[idxs], traj[idxs, 10], **interp_kwargs
            )
            self.interp_wy = scipy.interpolate.interp1d(
                times[idxs], traj[idxs, 11], **interp_kwargs
            )
            self.interp_wz = scipy.interpolate.interp1d(
                times[idxs], traj[idxs, 12], **interp_kwargs
            )
...
if __name__ == "__main__":
    ...
    for i, label in enumerate(
        ...
        axs[1].scatter(subtimes, subtraj[:, i], c=color, label=label, s=1.0)
