...
class MPPIIRL:
    ...
    def __init__(self, expert_dataset, mppi):
        ...
        self.mppi = mppi
        self.weights = torch.zeros(len(expert_dataset.feature_keys))
...
#            learner_feature_counts = self.expert_dataset.get_feature_counts(traj, map_features, map_metadata)
        ...
        self.weights -= self.lr * grads.mean(dim=0)

        print('__________ITR {}__________'.format(self.itr))
        print('WEIGHTS:\n', np.stack([np.array(self.expert_dataset.feature_keys), self.weights.numpy()], axis=-1))
