...
if __name__ == '__main__':
    ...
    parser.add_argument('--experiment_fp', type=str, required=True, help='Dir pointing to experiment itrs')
    ...
    parser.add_argument('--frame_skip', type=int, required=False, default=10, help='number of frames to skip in eval')
    parser.add_argument('--itr_skip', type=int, required=False, default=5, help='number of itrs to skip')
    ...
    model = torch.load(os.path.join(args.experiment_fp, model_fps[0]), map_location='cpu')
    ...
    for model_fp in model_fps:
        model = torch.load(os.path.join(args.experiment_fp, model_fp), map_location='cpu').to(args.device)
        model.network.eval()
        ...
        torch.save(experiments_res, save_fp) #deliberate decision to save after every model eval
