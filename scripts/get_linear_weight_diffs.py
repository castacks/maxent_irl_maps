...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_dir', type=str, required=True, help='dir of linear weights')
...
#        ens_w = [x.weight.squeeze().detach().cpu() for x in torch.load(fp).network.cost_heads]
    ...
    weight_diffs = torch.linalg.norm(weights.mean(dim=1)[1:] - weights.mean(dim=1)[:-1], dim=-1)
