...
def run_preproc_traj(traj_fp, dst_fp, config):
    ...
    for i in tqdm.tqdm(range(len(gridmap_ts))):
        ...
        if not all([validate_sample_times(target_times, src) for src in [odom_ts, steer_angle_ts, img_ts, pcl_ts]]):
            ...
            vlad_map_data = np.load(os.path.join(traj_fp, config['local_vlad_map'], '{:06d}_data.npy'.format(i)))
            vlad_map_metadata = yaml.safe_load(open(os.path.join(traj_fp, config['local_vlad_map'], '{:06d}_metadata.yaml'.format(i)), 'r'))
            vlad_map_feature_keys = []
            for k in range(vlad_map_data.shape[0]):
                vlad_map_feature_keys.append('VLAD_' + str(k))

            semantic_map_data = np.load(os.path.join(traj_fp, config['local_semantic_map'], '{:06d}_data.npy'.format(i)))
            semantic_map_metadata = yaml.safe_load(open(os.path.join(traj_fp, config['local_semantic_map'], '{:06d}_metadata.yaml'.format(i)), 'r'))
            ...
            res = {
                ...
                'gridmap_feature_keys': gridmap_feature_keys + dino_map_feature_keys + vlad_map_feature_keys + semantic_map_feature_keys,
            }
