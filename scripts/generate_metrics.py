...
from maxent_irl_maps.metrics.speedmap_metrics import *
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mppi_eval_fp", type=str, required=True, help="mppi config to eval on"
    )
    parser.add_argument(
        "--viz",
        action="store_true",
        required=False,
        help="set this flag to visualize output",
    )
    parser.add_argument(
        "--use_planner",
        action="store_true",
        required=False,
        help="set this if optimizer is planner",
    )
    parser.add_argument(
        "--alter",
        action="store_true",
        required=False,
        help="set this to run Alter baseline",
    )
    parser.add_argument(
        "--semantics",
        action="store_true",
        required=False,
        help="set this to run semantics baseline",
    )
    parser.add_argument(
        "--terrainnet",
        action="store_true",
        required=False,
        help="set this to run terrainnet",
    )
    parser.add_argument(
        "--device",
        type=str,
    ...
    if args.use_planner:
        res_speed = get_speedmap_metrics(
            model, frame_skip=1, viz=args.viz, save_fp=args.save_fp
        )
        res = get_metrics_planner(
            model, metrics, frame_skip=1, viz=args.viz, save_fp=args.save_fp
        )
