...
if __name__ == "__main__":
    ...
    for i in range(args.n):
        ...
        with torch.no_grad():
            speedmap_probs = res.network.forward(dpt["map_features"].unsqueeze(0))[
                "speedmap"
            ][0].softmax(dim=0)
        ...
        for q, ax in zip(quantiles, axs[1:]):
            speedmap = compute_speedmap_quantile(
                speedmap_cdf, res.network.speed_bins.to(args.device), q
            )

            ax.imshow(
                speedmap.cpu().T,
                origin="lower",
                vmin=res.network.speed_bins[0],
                vmax=res.network.speed_bins[-1],
                cmap="jet",
            )
