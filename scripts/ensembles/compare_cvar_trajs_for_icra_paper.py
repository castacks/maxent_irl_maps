...
def visualize_cvar(model, idx):
    ...
    with torch.no_grad():
        ...
        for ax, cm, traj, cost, label, color in zip(axs2, costmaps, trajs, costs, labels, colors):
            if label == 'Occupancy':
                r = ax.imshow(cm.cpu(), origin='lower', cmap='plasma', extent=(xmin, xmax, ymin, ymax))
            else:
                r = ax.imshow(cm.cpu(), origin='lower', cmap='plasma', extent=(xmin, xmax, ymin, ymax), vmin=vmin, vmax=vmax)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--skip', type=int, required=False, default=1, help='process very k-th frame')
    parser.add_argument('--device', type=str, required=False, default='cpu', help='device to run on ')
    ...
    # set up batch MPPI
    cfn = BatchMultiWaypointCostMapCostFunction(unknown_cost=model.mppi.cost_fn.unknown_cost, goal_cost=model.mppi.cost_fn.goal_cost)
