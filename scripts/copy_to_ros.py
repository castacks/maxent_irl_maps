...
if __name__ == '__main__':
    ...
    parser.add_argument('--src_dir', type=str, required=True, help='path to expeiment to use for ros')
    parser.add_argument('--dst_dir', type=str, required=True, help='path to save model to')
    ...
    params['dataset']['params']['root_fp'] = os.path.join(args.dst_dir, exp_name, 'dataset')

    #copy planner params and normalizations
    shutil.copy(params['solver']['planner_fp'], os.path.join(args.dst_dir, exp_name, 'planner_config.yaml'))
    params['solver']['planner_fp'] = os.path.join(args.dst_dir, exp_name, '_planner_config.yaml')

    yaml.dump(params, open(os.path.join(args.dst_dir, exp_name, '_run_params.yaml'), 'w'))
