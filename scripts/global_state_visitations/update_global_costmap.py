...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--costmap_fp", type=str, required=True, help="location to get costmap"
    )
    parser.add_argument(
        "--utm_zone",
        type=int,
        required=False,
        default=17,
        help="UTM zone that the data is in (probably 17 if CMU data)",
    )
    parser.add_argument("--bag_fp", type=str, required=True, help="dir of bags to add")
    parser.add_argument(
        "--preprocess_fp", type=str, required=True, help="dir of bags to add"
    )
    parser.add_argument(
        "--map_topic",
        type=str,
    ...
    parser.add_argument(
        ...
        required=False,
        default="cpu",
        help="device to run costmap inference on",
    )
    ...
    dataset = MaxEntIRLDataset(
        ...
        horizon=buf.model.expert_dataset.horizon,
        feature_keys=buf.model.expert_dataset.feature_keys,
    ).to(args.device)
