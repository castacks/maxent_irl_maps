...
def get_lat_long_grid(gsv, utm_zone):
    ...
    proj = Proj(
        "+proj=utm +zone={} +north +ellps=WGS84 +datum=WGS84 +units=m +no_defs".format(
            utm_zone
        )
    )
    grid = np.stack(
        np.meshgrid(
            np.arange(ox, ox + dx, res), np.arange(oy, oy + dy, res), indexing="ij"
        ),
        axis=-1,
    )
...
def plot_gsv(gsv, utm_zone, fig=None, axs=None, plot_speed=False):
    ...
    grid_px = np.stack(
        [grid_px_df["y_pixel"].to_numpy(), grid_px_df["x_pixel"].to_numpy()], axis=-1
    )
    ...
    if mask.sum() > 0:
        r1 = axs[0].scatter(
            mask_px[:, 1], mask_px[:, 0], s=1.0, c=np.log(mask_val), cmap=cmap
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--utm_zone", type=int, required=False, default=17, help="UTM zone for data"
    )
