...
def hausdorff_distance(A, B, directed_distance=0, symmetric_distance=0):
    ...
    """
    ab_dist_matrix = torch.linalg.norm(A.unsqueeze(1)[..., :2] - B.unsqueeze(0)[..., :2], dim=-1)
    ...
    if symmetric_distance == 0:
        ...
        dist = (A_to_B_dist * A.shape[0] + B_to_A_dist * B.shape[0]) / (A.shape[0] + B.shape[0])

    return dist

if __name__ == '__main__':
    kbm = SteerSetpointThrottleKBM(L=3.0, throttle_lim=[1.5, 3.5], steer_lim=[-0.52, 0.52], steer_rate_lim=0.2, dt=0.15)

    X0 = torch.tensor([0., 0., 0., 3.5, 0.])
...
#        action_seqs[i, :, 1] = s
    ...
    trajs = torch.stack([kbm.rollout(X0, action_seqs[i]) for i in range(action_seqs.shape[0])], dim=0)
    ...
    for i in range(traj_dists.shape[1]):
        ...
        for j, traj in enumerate(trajs):
            ...
            axs[0].text(traj[-1, 0], traj[-1, 1], '{}, ({:.2f})'.format(j, traj_dists[j, i]))
