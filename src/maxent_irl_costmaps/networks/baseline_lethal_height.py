...
class LethalHeightCostmap(torch.nn.Module):
    ...
    """
    def __init__(self, dataset, lethal_height=0.5, blur_sigma=2.0, sharpness=2.0, clip_low=0.8):
        """
        Args:
...
#        self.cost = 1e8
    ...
    def forward(self, map_features):
        ...
        costmap = (diffs > self.lethal_height).cpu().numpy().astype(np.float32) #find cells above lethal height
