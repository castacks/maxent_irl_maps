...
class ResnetCostmapCategoricalSpeedmapCNNEnsemble3(nn.Module):
    def __init__(self, in_channels, hidden_channels, speed_nbins=20, max_speed=10., ensemble_dim=100, hidden_activation='tanh', dropout=0.0, activation_type='sigmoid', activation_scale=1.0, device='cpu'):
        ...
        self.speed_head = nn.Conv2d(in_channels=self.channel_sizes[-2], out_channels=self.speed_nbins * self.ensemble_dim, kernel_size=1, bias=True)
    ...
    def forward(self, x, return_features=True):
        ...
        speed_logits = speed_logits.view(*batch_dims, self.ensemble_dim, self.speed_nbins, *map_dims)
...
class ResnetCostmapCategoricalSpeedmapCNNEnsemble2(nn.Module):
    ...
    def ensemble_forward(self, x, return_features=True, ne=-1):
        ...
        features_batch = torch.stack([layer.forward(x) for layer in base_layers], dim=-4)
