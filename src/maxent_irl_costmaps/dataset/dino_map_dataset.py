...
class DinoMapDataset(Dataset):
    ...
    def __init__(self, fp, positional_n=8, dino_n=-1, device='cpu'):
        ...
        self.metadata = yaml.safe_load(open(os.path.join(self.fp, self.dfps[0], 'map_metadata.yaml'), 'r'))
        ...
        self.dino_n = np.load(os.path.join(self.fp, self.dfps[0], 'dino_map_data.npy')).shape[0]
    ...
    def preprocess(self):
        ...
        torch.save({k:v.cpu() for k,v in self.normalizations.items()}, os.path.join(self.fp, 'normalizations.pt'))
    ...
    def get_positional_embedding(self):
        ...
        fft_as = torch.randn(self.positional_n, device=self.device).view(self.positional_n, 1, 1)
        fft_bs = torch.randn(self.positional_n, device=self.device).view(self.positional_n, 1, 1)
    ...
    def get_unnormalized_dpt(self, idx=-1):
        ...
        image = cv2.imread(os.path.join(fp, 'image.png')) / 255.
        dino_map_data = torch.tensor(np.load(os.path.join(fp, 'dino_map_data.npy'))[:self.dino_n], device=self.device, dtype=torch.float32)
        geom_map_data = torch.tensor(np.load(os.path.join(fp, 'geometric_map_data.npy')), device=self.device, dtype=torch.float32)
        ...
        metadata['length_x'] = torch.tensor(metadata['length_x'], device=self.device, dtype=torch.float32)
        metadata['length_y'] = torch.tensor(metadata['length_y'], device=self.device, dtype=torch.float32)
        metadata['height'] = torch.tensor(metadata['length_x'], device=self.device, dtype=torch.float32)
        metadata['width'] = torch.tensor(metadata['length_y'], device=self.device, dtype=torch.float32)
        metadata['resolution'] = torch.tensor(metadata['resolution'], device=self.device, dtype=torch.float32)
        ...
        return {
            ...
            'steer': steer,
            'traj': traj,
            'image': torch.tensor(image, dtype=torch.float32, device=self.device).permute(2,0,1)[[2,1,0]],
    ...
    def get_normalized_dpt(self, idx=-1):
        ...
        udpt['map_features'] = ((udpt['map_features'] - _mu) / _sig).clip(-10., 10.)
    ...
    def visualize(self, dpt, fig=None, axs=None):
        ...
        if self.dino_n >= 3:
            ...
            axs[2].imshow(dino_viz.permute(2,1,0).cpu(), origin='lower', extent=extent)
