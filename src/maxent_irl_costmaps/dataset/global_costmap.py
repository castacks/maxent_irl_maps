...
class GlobalCostmap:
    def __init__(self, config_fp, model_fp, bag_dir=None, odom_topic='/integrated_to_init', gps_topic='/odometry/filtered_odom', map_features_topic='/local_gridmap', dt=0.05, device='cpu'):
        """
        Args:
    ...
    def check_trajectory(self, tfp):
        ...
        #check for gps jumps 
        diffs = np.linalg.norm(traj['gps_traj'][1:, :3] - traj['gps_traj'][:-1, :3], axis=-1)
    ...
    def add_dataset(self, dataset):
        ...
        for i, batch in enumerate(dataset):
            ...
            map_poses = torch.stack(torch.meshgrid([map_dxs, map_dys], indexing='ij'), dim=-1).to(gps_pose.device) # W x H x 2
            R = torch.tensor([[pose_rot.cos(), -pose_rot.sin()], [pose_rot.sin(), pose_rot.cos()]]).to(gps_pose.device)

            gps_map_poses = (gps_pose[:2].view(1, 1, 2, 1) + torch.matmul(R.view(1, 1, 2, 2), map_poses.unsqueeze(-1))).squeeze(-1) #[W x H x 2]
            ...
            ixs = ((px_raster - self.metadata['origin'][0]) / self.metadata['resolution']).long()
            iys = ((py_raster - self.metadata['origin'][1]) / self.metadata['resolution']).long()
            ...
            dists = torch.linalg.norm(gps_pose[:2].view(1, 2) - torch.stack([px_raster, py_raster], dim=-1), dim=-1)
            ...
            if i % 10000 == 0 or (i+2 == len(dataset)):
                ...
                axs[2].imshow(self.minimum_distances.clamp(0., 100.), cmap='plasma')
...
#        pixel_coordinates_flipped = pixel_coordinates.swapaxes(-2,-3)
        ...
        map_p_high = torch.tensor(self.state_visitations.shape).to(self.device).view(1, 1, 1, 2)
